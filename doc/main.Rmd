...
if(run.cv_dnn){
  ...
  res_cv_dnn <- matrix(0, nrow = length(lr), ncol = 6)
  count = 0
...
if(run.cv_dnn){
  p1 <- res_cv_dnn %>%
    ggplot(aes(x = lr, y = mean_error,
               ymin = mean_error - sd_error, ymax = mean_error + sd_error)) +
    geom_crossbar() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
  
  p2 <- res_cv_dnn %>%
    ggplot(aes(x = lr, y = mean_AUC,
               ymin = mean_AUC - sd_AUC, ymax = mean_AUC + sd_AUC)) +
    geom_crossbar() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
cat("The accuracy of model:", "GDNN with lr =", par_lr_best, "is", accu*100, "%.\n")
