---
title: "Main"
author: "Amir Idris, Catherine Gao, Eve Washington, Ruize Yu, Yiwen Fang"
output:
...
tm_train <- NA
if (sample.reweight){
  tm_train <- system.time(fit_train_gbm <- train_gbm(feature_train, label_train, w = weight_train, par_n.trees_best, par_shrinkage_best))
} else {
  tm_train <- system.time(fit_train_gbm <- train_gbm(feature_train, label_train, w = NULL, par_n.trees_best, par_shrinkage_best))
}
...
if(run.test_gbm){
  ...
  tm_test <- system.time({prob_pred <- test_gbm(fit_train_gbm, feature_test, par_n.trees_best, pred.type = 'response');})
}
```
...
Since DNN training takes very long time in K folds, we do not show the whole tuning process here.
```{r runcv_dnn}
...
if(run.cv_dnn){
  ...
  res_cv_dnn <- matrix(0, nrow = length(lrs), ncol = 5)
  count = 0
...
tm_train <- NA
if (sample.reweight){
  tm_train <- system.time(fit_train_dnn <- train_dnn(feature_train, label_train, w = weight_train, feature_test, label_test, weight_test, par_lr_best))
} else {
  tm_train <- system.time(fit_train_dnn <- train_dnn(feature_train, label_train, w = NULL, feature_test, label_test, weight_test = NULL, par_lr_best))
}
