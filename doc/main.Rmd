...
feature_train = as.matrix(feature_train_pca[, -6007])
...
```{r test}
tm_test = NA
feature_test <- as.matrix(feature_test_pca[, -6007])
if(run.test){
  ...
  tm_test <- system.time(prob_pred <- test(fit_train, as.data.frame(feature_test_pca), pred.type = 'response'))
}
```
...
cat("Time for pca=", tm_feature_train_pca[1], "s \n")
