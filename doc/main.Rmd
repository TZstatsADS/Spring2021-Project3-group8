...
# }else{
#   load(file="../output/feature_train_pca.RData")
...
```{r runcv_LR}
source("../lib/cross_validation_LR.R")
feature_train = as.matrix(dat_train[, -6007])
label_train = as.integer(dat_train$label)
```
...
tm_train <- NA
if (sample.reweight){
  tm_train <- system.time(fit_train <- train_LR(feature_train, label_train, w = weight_train))
} else {
  tm_train <- system.time(fit_train <- train_gbm(feature_train, label_train))
}
save(fit_train, file="../output/fit_train_LR.RData")
```
...
feature_test <- as.matrix(dat_test[, -6007])
if(run.test_gbm){
  load(file="../output/fit_train_LR.RData")
  tm_test <- system.time({prob_pred <- test_gbm(fit_train, feature_test, pred.type = 'response');})
}
```
...
cat("The accuracy of model:", "LR is", accu*100, "%.\n")
cat("The AUC of model:", "LR is", auc, ".\n")
