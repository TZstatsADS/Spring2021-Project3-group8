...
### Author: Amir Idris
### Project 3
### -structure from Yiwen's cross_validation_gbm


cv.function_LR <- function(features, labels, K, reweight = FALSE){
  ### Input:
  ...
  for (i in 1:K){
    ...
    ## model training
    if (reweight){
      model_train <- train_LR(feature_train, label_train, w = weight_train)
    } else {
      model_train <- train_LR(feature_train, label_train)
    }
    
    ## make predictions
    prob_pred <- test_LR(model_train, feature_test, pred.type = 'response')
